    ...
    try:
        ...
        if request.method == 'POST':
            if request.POST.get('submit') == str(lst[-1]):
                ...
                respo = Response(question=pre_question, user=getuser.user, selected_answer=user_input, score=score, percentage=percentage)
                respo.save()
        ...
        getuser.save()
        return render(request, 'task2part2temp/question.html', {'user': getuser, 'question': question, 'time': [msg2], 'increment':increment, 'percentage':percentage})
    except:
